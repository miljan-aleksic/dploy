...
module.exports = {
  ...
  confirm(settings) {
    ...
    queue.upload.forEach((item) => {
      ...
      const remote = path.join(settings.path.remote, item.remote)
      logger.spaced(3, `${chalk.white(local)} → ${chalk.green(remote)}`);
    });

    queue.delete.forEach((item) => {
      const local = item.local
      const remote = path.join(settings.path.remote, item.remote)
      logger.spaced(3, `${chalk.white(local)} → ${chalk.red(remote)}`);
    });
