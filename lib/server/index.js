...
const isRelative = require('../util/is-relative');
const progressBar = require('../util/progress-bar');
const parseRevision = require('../util/parse-revision');
  ...
  uploadQueue(settings) {
    ...
    settings.queue.upload.forEach((file) => {
      ...
      actions.push(() => {
        ...
        const remote = path.join(settings.path.remote, file.remote)

        bar.tick({ 'file': local });

        return service.put(local, remote).then((res) => {

          logger.debug('File uploaded:', chalk.white(`${local} → ${remote}`));
          result.success.push(res);

        }).catch((res) => {

          logger.error('File not uploaded:', chalk.white(`${local} → ${remote}`), res.reason);
          result.fail.push(res);
